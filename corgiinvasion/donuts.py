...
class Donut(Sprite):
    """A class to manage donuts fallind down."""
    ...
    def blitme(self):
        """Draw the donut to the screen."""
        self.screen.blit(self.image, self.rect)

    def update(self, stats, height):
        """Move the donut up the screen down."""
        self.y += self.speed_factor + stats.donut_speed_level
        self.rect.y = self.y
        # Get rid of donuts that have disappeared.
        if self.rect.y > height:
            self.kill()


def create_donuts(screen, donuts, donut_number):
    """Create a donut, and place it in the row."""
    donut = Donut(screen)
...
def get_number_donuts_x(width, donut_width):
    """Determine the number of donuts that fit in a row."""
...
def create_fleet(screen, donuts, width, stats):
    """Create a full fleet of donuts."""
    # Create a donut, and find number of donuts in a row.
    donut = Donut(screen)
    number_donuts_x = get_number_donuts_x(width, donut.rect.width)
    # Create the fleet of donuts randomly due to level.
...
def update_donuts(screen, donuts, width, height, stats):
    """ Update a donut's movement or create a new fleet of donuts"""
    if donuts:
